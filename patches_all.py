...
for K_samples in K_samples_ :
    ...
    for iterations in range(1):
        for i in [-1]:
            ...
            lower_bound = upper_bound = bound_updated_encoder = bound_updated_test_encoder = pseudo_iwae = m_iwae = xm_iwae = xm_NN_iwae = iaf_iwae = z_iwae = mixture_iwae = 0
            total_loss = 0
            ...
            for data in test_loader:
                ...
                x_logits_pseudo_gibbs, x_sample_pseudo_gibbs, iwae = pseudo_gibbs(sampled_image.to(device,dtype = torch.float), b_data.to(device,dtype = torch.float), b_mask, encoder, decoder, p_z, d, results, iterations, T=num_epochs_test, nb=nb, K = 1, full = b_full.reshape([1,1,28,28]).to(device,dtype = torch.float))
                end_pg = datetime.now()
                ...
                m_nelbo, m_error, x_full_logits, m_loglike, iwae =  m_g_sampler(iota_x = sampled_image_m.to(device,dtype = torch.float), full = b_full.reshape([1,1,28,28]).to(device,dtype = torch.float), mask = b_mask,encoder = encoder,decoder = decoder, p_z= p_z, d=d, results = results, nb = nb, iterations = iterations, K= 1, T=num_epochs_test)
                end_m = datetime.now()
                ...
                xm_nelbo_, xm_error_, iwae = optimize_IAF(num_epochs = num_epochs_test, z_params = z_init, b_data = b_data.to(device,dtype = torch.float), sampled_image_o = sampled_image_o.to(device,dtype = torch.float), b_mask = b_mask.to(device,dtype = torch.bool), b_full = b_full.to(device,dtype = torch.float), p_z = p_z, encoder = encoder, decoder = decoder, device = device, d = d, results = results, iterations = iterations, nb=nb, K_samples = K_samples, p_z_eval = p_z_eval  )
                ...
                z_nelbo_, z_error_, iwae = optimize_mixture(num_epochs = num_epochs_test, p_z = p_z, b_data = b_data.to(device,dtype = torch.float), b_full = b_full.to(device,dtype = torch.float), b_mask = b_mask.to(device,dtype = torch.bool), encoder = encoder, decoder = decoder, device = device, d = d, results = results, iterations = iterations, nb=nb , K_samples = K_samples)
                ...
                print(lower_bound, upper_bound, bound_updated_encoder, pseudo_iwae, m_iwae,  xm_iwae,  xm_NN_iwae,  iaf_iwae, z_iwae, mixture_iwae)

file_10_images = results + str(-1) + "/10img-stats.pkl"
with open(file_10_images, 'wb') as file:
    pickle.dump([xm_loss,xm_loss_NN,z_loss,iaf_loss,mixture_loss ], file)
...
print(lower_bound/nb, upper_bound/nb, bound_updated_encoder/nb, pseudo_iwae/nb, m_iwae/nb,  xm_iwae/nb,  xm_NN_iwae/nb,  iaf_iwae/nb, z_iwae/nb, mixture_iwae/nb)
