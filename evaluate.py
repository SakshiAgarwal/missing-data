...
for data in test_loader:
	...
	lower, img= eval_baseline(max_samples, p_z, encoder, decoder, b_data.to(device,dtype = torch.float), b_full.to(device,dtype = torch.float), b_mask.to(device,dtype = torch.bool), d)
	lower_bound += lower
	#images.append(img)

	upper, img = eval_baseline(max_samples, p_z, encoder, decoder, b_full.to(device,dtype = torch.float), b_full.to(device,dtype = torch.float), b_mask.to(device,dtype = torch.bool), d)
	upper_bound += upper
	#images.append(img)

	updated_encoder, img = eval_baseline(max_samples, p_z, encoder_updated, decoder, b_data.to(device,dtype = torch.float), b_full.to(device,dtype = torch.float), b_mask.to(device,dtype = torch.bool), d)
	bound_updated_encoder += updated_encoder
	images.append(img)

	updated_test_encoder, img = eval_baseline(max_samples, p_z, encoder_updated_test, decoder, b_data.to(device,dtype = torch.float), b_full.to(device,dtype = torch.float), b_mask.to(device,dtype = torch.bool), d)
	bound_updated_test_encoder += updated_test_encoder
	images.append(img)

	pseudo_gibbs_, img = evaluate_pseudo_gibbs(max_samples, p_z, encoder, decoder, pseudo_gibbs_sample[i-1].to(device,dtype = torch.float), b_data.to(device,dtype = torch.float),  b_full.to(device,dtype = torch.float), b_mask.to(device,dtype = torch.bool), d, device)
	pseudo_gibbs_iwae += pseudo_gibbs_
	images.append(img)

	metropolis_within_gibbs_, img = evaluate_metropolis_within_gibbs(max_samples, p_z, encoder, decoder, metropolis_gibbs_sample[i-1].to(device,dtype = torch.float), b_data.to(device,dtype = torch.float),  b_full.to(device,dtype = torch.float), b_mask.to(device,dtype = torch.bool), d, device)
	metropolis_within_gibbs_iwae += metropolis_within_gibbs_
	images.append(img)

	z_, img = evaluate_z(p_z = p_z, b_data = b_data.to(device,dtype = torch.float),  b_full = b_full.to(device,dtype = torch.float), b_mask = b_mask.to(device,dtype = torch.bool),z_params = z_params[i-1], decoder = decoder, device = device, d = d, results = results, nb=nb , K_samples = max_samples )
	z_iwae += z_
	images.append(img)

	iaf_, img = evaluate_iaf(p_z = p_z, b_data = b_data.to(device,dtype = torch.float), b_full = b_full.to(device,dtype = torch.float), b_mask = b_mask.to(device,dtype = torch.bool),iaf_params = iaf_params[i-1], encoder = encoder, decoder = decoder, device = device, d = d, results = results, nb=nb , K_samples = max_samples )
	iaf_iwae += iaf_
	images.append(img)

	mixture_, img =  evaluate_z(p_z = p_z, b_data = b_data.to(device,dtype = torch.float), b_full = b_full.to(device,dtype = torch.float), b_mask = b_mask.to(device,dtype = torch.bool),z_params = mixture_params[i-1], decoder = decoder, device = device, d = d, results = results, nb=nb , K_samples = max_samples, ismixture=True )
	mixture_iwae += mixture_
	images.append(img)	

	mixture_inits_, img =  evaluate_z(p_z = p_z, b_data = b_data.to(device,dtype = torch.float), b_full = b_full.to(device,dtype = torch.float), b_mask = b_mask.to(device,dtype = torch.bool),z_params = mixture_params_inits[i-1], decoder = decoder, device = device, d = d, results = results, nb=nb , K_samples = max_samples, ismixture=True )
