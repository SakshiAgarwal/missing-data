    ...
    for i in [-1]:
        ...
        for data in test_loader:
            ...
            if dd:
                ...
                z_nelbo_, z_error_ = optimize_z(num_epochs = num_epochs_test, xm_params = xm_params, z_params = z_init, p_z = p_z, b_data = b_data.to(device,dtype = torch.float), b_full = b_full.to(device,dtype = torch.float),  sampled_image_o = sampled_image_o.to(device,dtype = torch.float), b_mask = b_mask.to(device,dtype = torch.bool),  encoder = encoder, decoder = decoder, device = device, d = d, results = results, iterations = iterations, nb=nb , K_samples = K_samples, data='svhn')
                end_z = datetime.now()
