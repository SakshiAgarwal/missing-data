...
for data in train_loader:
    b_data, b_mask = data
    plot_image_svhn(np.squeeze(b_data[0].cpu().data.numpy()),results  + "generated-images/true_image.png")
    break
...
for i in range(100):
    x = generate_samples(p_z, decoder, d, L=1, model = "sigma-vae").cpu().data.numpy().reshape(3,32,32)  
    plot_image_svhn(x, file= results  + "generated-images/generated_image" + str(i) + ".png")
...
for iterations in range(6):
    for i in [-1]:
        ...
        for data in test_loader:
            ...
            upper_imp = mvae_impute(iota_x = b_data,mask = b_mask,encoder = encoder,decoder = decoder, p_z = p_z, d=d, L=1)[0].reshape(1,1,28,28)
            ...
            lower_imp = mvae_impute(iota_x = b_full,mask = b_mask,encoder = encoder,decoder = decoder, p_z = p_z, d=d, L=1)[0].reshape(1,1,28,28)
            ...
            if random: 
                ...
                x_logits_init, b_data_init, z_init = burn_in(b_data, b_mask, encoder, decoder, p_z, d, burn_in_period=burn_in_period)
            else:
                b_data_init = b_data
            ...
            if not burn_in:
                ...
                if iterations==0:
                    ...
                    burn_in_image[~b_mask] = torch.sigmoid(x_logits_init)[~b_mask]
            ...
            m_nelbo, m_error, x_full_logits, m_loglike =  m_g_sampler(iota_x = sampled_image_m, full = b_full, mask = b_mask,encoder = encoder,decoder = decoder, p_z= p_z, d=d, K=1, T=num_epochs_test)
            ...
            test_optimizer = torch.optim.Adam([xm_params], lr=0.0001, betas=(0.9, 0.999)) 
            ...
            plot_images_z(image1,image2, image3, results + str(i) + "/images/" +  str(nb%10) + "/"  + str(iterations) + '-' , image4, image5, results + str(i) + "/compiled/image-z" + str(nb%10) + str(iterations) +".pdf")
            ...
            zgivenx = q_zgivenxobs.rsample()
            zgivenx_flat = zgivenx.reshape([1,d])
            xm_params = decoder.forward(zgivenx_flat).reshape([1,1,28,28])[~b_mask].detach()
            ...
            for k in range(num_epochs):
                ...
                loss, log_like, aa, bb, cc = xm_loss_q(iota_x = b_data, sampled = sampled_image_o, mask = b_mask,p_z = p_z, z_params = z_params, xm_params = xm_params, encoder = encoder, decoder = decoder , device= device, d=d,  K=1, K_z= 1, epoch=k)
                loss.backward()            
                test_optimizer.step()
                ...
                imputation[~b_mask] = v
                loss, log_like, aa, bb, cc = xm_loss_q(iota_x = imputation, sampled = imputation, mask = b_mask,p_z = p_z, z_params = z_params, xm_params = xm_params, encoder = encoder, decoder = decoder , device= device, d=d,  K=1, K_z= 1, train=False, epoch=k)
                ...
                xm_mse[k] += err
                loss, log_like, aa, bb, cc = xm_loss_q(iota_x = b_data, sampled = sampled_image_o, mask = b_mask,p_z = p_z, z_params = z_params, xm_params = xm_params, encoder = encoder, decoder = decoder , device= device, d=d, K=100, K_z= 1, epoch=k)
                term1 += aa.item()
                term2 += bb.item()
                term3 += cc.item()
