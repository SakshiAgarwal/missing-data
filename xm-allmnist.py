...
for i in [-2]:
    ...
    for j in [50]: #10,100
        test_loader = torch.utils.data.DataLoader(dataset=BinaryMNIST_Test(binarize = binary_data, noise=True, std=j),batch_size=1)
        ...
        for data in test_loader:
            ...
            plot_image(np.squeeze(b_data.cpu().data.numpy()),results + str(i) + "/images/" + str(nb%10) + "/noise" + str(j)  + "burn-in.png" )
            ...
            plot_image(np.squeeze(pseudo_gibbs_image.cpu().data.numpy()), results + str(i) + "/images/" + str(nb%10) + "/noise" + str(j) +"pseudo-gibbs.png" )
            ...
            plot_image(np.squeeze(metropolis_image.cpu().data.numpy()), results + str(i) + "/images/" + str(nb%10) + "/noise" + str(j) + "metropolis-within-pseudo-gibbs.png" )
            ...
            for k in range(num_epochs_test):
                print(k)
                torch.nan_to_num(xm_params)
                beta = beta/2 + 0.5
                test_optimizer.zero_grad()
                loss, log_like, aa, bb, cc, dd, ee = xm_loss_noise(iota_x = b_data,mask = b_mask,p_z = p_z, xm_params = xm_params, encoder = encoder, decoder = decoder , device= device, d=d, beta=beta,K=1, K_z= 1, std=j)
                loss.backward()            
                test_optimizer.step()
                ...
                loss, log_like, aa, bb, cc, dd, ee = xm_loss_noise(iota_x = imputation,mask = b_mask,p_z = p_z, xm_params = xm_params, encoder = encoder, decoder = decoder , device= device, d=d, beta=beta, K=1, K_z= 1, std=j)
                xm_approx_loss[k] += log_like.item()
                imputation = imputation.cpu().data.numpy().reshape(28,28)

                loss, approx_loss, aa, bb, cc, dd, ee = xm_loss_noise(iota_x = b_data,mask = b_mask,p_z = p_z, xm_params = xm_params, encoder = encoder, decoder = decoder , device= device, d=d, beta=beta, K=100, K_z= 1, std=j)
                ...
                #print(loss_acc)
                if (k+1)==num_epochs_test or k==10 or (k)%100==0:
                    with torch.no_grad():
                        img = b_full.cpu().data.numpy()         ## added .data
                        plot_image(np.squeeze(img),results + str(i) + "/images/" + str(nb%10) + "/noise" + str(j) + "xm-true.png" )
                        b_data[~b_mask] = b_data_init[~b_mask]      
                        img = b_data.cpu().data.numpy() 
                        plot_image(np.squeeze(img),results + str(i) + "/images/" + str(nb%10) + "/noise" + str(j) + "missing.png" )
                        imputation = b_data 
                        imputation[~b_mask] = torch.sigmoid(xm_params) 
                        ...
                        plot_image(np.squeeze(img),results + str(i) + "/images/" +  str(nb%10) + "/noise" + str(j) + "iter-" + str(k) +"xm-input.png")
                        ...
                        if binary_data:
                            xhat[xhat < 0.5] = 0
                            xhat[xhat >= 0.5] = 1
                        #plot_image(np.squeeze(xhat), resultss + str(i) + "/images/" + str(nb%10) + "/xm-imputed.png" )
                #test_mse += float(err)
            ...
            if (nb==5):
                ...
                plt.ylabel('Loss')
                plt.show()
                plt.savefig(results + str(i) + "/loss/" +  "noise" + str(j) + 'loss-iteration.png')
                plt.close()
                ...
                plt.show()
                plt.savefig(results + str(i) + "/" + "noise" + str(j) + 'comparison-loglikelihood.png')
                plt.close()
                ...
                #plt.ylim(0, 0.2)
                plt.show()
                plt.savefig(results + str(i) + "/" + "noise" + str(j) +'comparison-mse.png')
                plt.close()
                ...
                plt.ylabel('term1')
                plt.show()
                plt.savefig(results + str(i) + "/loss/" + "noise" + str(j) + 'loss-term1.png')
                plt.close()
                ...
                plt.ylabel('term2')
                plt.show()
                plt.savefig(results + str(i) + "/loss/" + "noise" + str(j) +'loss-term2.png')
                plt.close()
                ...
                plt.ylabel('term3')
                plt.show()
                plt.savefig(results + str(i) + "/loss/" + "/noise" + str(j) +'loss-term3.png')
                plt.close()
                loss_acc *= nb
