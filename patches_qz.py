    ...
    for i in [-1]:
        ...
        for data in test_loader:
            ...
            if random: 
                ...
                x_logits_init, b_data_init, z_init = burn_in(b_data.to(device,dtype = torch.float), b_mask, encoder, decoder, p_z, d, burn_in_period=burn_in_period)
            ...
            x_logits_pseudo_gibbs, x_sample_pseudo_gibbs = pseudo_gibbs(sampled_image.to(device,dtype = torch.float), b_data.to(device,dtype = torch.float), b_mask, encoder, decoder, p_z, d, T=num_epochs_test, nb=nb, K =1)
            ...
            m_nelbo, m_error, x_full_logits, m_loglike =  m_g_sampler(iota_x = sampled_image_m.to(device,dtype = torch.float), full = b_full.to(device,dtype = torch.float), mask = b_mask,encoder = encoder,decoder = decoder, p_z= p_z, d=d, K=1, T=num_epochs_test)
            ...
            plot_image(np.squeeze(torch.sigmoid(x_logits_init).reshape(28,28).cpu().data.numpy()), results + str(i) + "/images/" + str(nb%10) + "/" + str(iterations) + "after-metropolis.png" )
            ...
            autoregressive_nn =  AutoRegressiveNN(d, [d]).cuda()
            optimizer_iaf = torch.optim.Adam(list(autoregressive_nn.parameters()), lr=0.01)
            z_params = z_init.cuda()
            ...
            for k in range(num_epochs_z):       
                print("In IAF----")  
                loss, log_like, aa, bb, flow_dist = z_loss_(iota_x = b_data.to(device,dtype = torch.float), mask = b_mask, p_z = p_z, z_params = z_params, xm_params = xm_params.to(device,dtype = torch.float), encoder = encoder, decoder = decoder , device = device, d=d, K=5000, K_z=1, iaf=True, autoregressive_nn = autoregressive_nn)
                ...
                err = np.array([mse(img.reshape([1,1,28,28]),b_full.cpu().data.numpy(),b_mask.cpu().data.numpy().astype(bool))])
            ...
            display_images(decoder, flow_dist, d, results + str(i) + "/compiled/" + str(nb%10)  + str(iterations)  + '.png', k = 50)
            ...
            if num_epochs_z:
                for num in range(100):
                    x = generate_samples(q_zgivenxobs, decoder, d, L=1).cpu().data.numpy().reshape(1,1,28,28)  
                    plot_image(np.squeeze(x), results + str(i) + "/images/samples_qz/" + str(nb%10) + "/" + str(iterations) + '-'   + str(num) +  ".png" ) 
            ...
            zgivenx = q_zgivenxobs.rsample()
            zgivenx_flat = zgivenx.reshape([1,d])
            xm_params = decoder.forward(zgivenx_flat).reshape([1,1,28,28])[~b_mask]

            xm_init = b_data.to(device,dtype = torch.float)
            xm_init[~b_mask] = torch.sigmoid(xm_params).reshape(-1)
            plot_image(np.squeeze(xm_init.cpu().data.numpy()),results + str(i) + "/images/" + str(nb%10) + "/"  + str(iterations) + "init.png" )
            ...
            for k in range(num_epochs):
                ...
                loss, log_like, aa, bb, cc = xm_loss_q(iota_x = b_data.to(device,dtype = torch.float), sampled = sampled_image_o.to(device,dtype = torch.float), mask = b_mask,p_z = p_z, z_params = z_params, xm_params = xm_params, encoder = encoder, decoder = decoder , device= device, d=d, beta=beta, K=1, K_z= 1, epoch=k)
                loss.backward()            
                test_optimizer.step()
                ...
                imputation[~b_mask] = v
                loss, log_like, aa, bb, cc = xm_loss_q(iota_x = imputation.to(device,dtype = torch.float), sampled = imputation.to(device,dtype = torch.float), mask = b_mask,p_z = p_z, z_params = z_params, xm_params = xm_params, encoder = encoder, decoder = decoder , device= device, d=d, beta=beta, K=1, K_z= 1, train=False, epoch=k)
                ...
                xm_mse[k] += err
                loss, log_like, aa, bb, cc = xm_loss_q(iota_x = b_data.to(device,dtype = torch.float), sampled = sampled_image_o.to(device,dtype = torch.float), mask = b_mask,p_z = p_z, z_params = z_params, xm_params = xm_params, encoder = encoder, decoder = decoder , device= device, d=d, beta=beta, K=100, K_z= 1, epoch=k)
                ...
                if (k+1)==num_epochs_test or (k+1)%10==0:
                    with torch.no_grad():
                        imputation = b_data.to(device,dtype = torch.float) 
                        imputation[~b_mask] = torch.sigmoid(xm_params).reshape(-1) 
                        img = imputation.cpu().data.numpy()
                        #plot_image(np.squeeze(img),results + str(i) + "image1/epoch_" +   str(k+1) + "xm-input.png", missing_pattern_x = missing_pattern_x, missing_pattern_y = missing_pattern_y )
